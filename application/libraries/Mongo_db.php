...
Class Mongo_db {
    ...
    private function connect() {
        ...
        try {
            ...
            $url = 'mongodb://'.$this->config[$this->activate]['username'].':'.$this->config[$this->activate]['password'].'@'.$this->config[$this->activate]['hostname'].':'.$this->config[$this->activate]['port'].',digiebot-shard-00-01-bhelp.mongodb.net:'.$this->config[$this->activate]['port'].',digiebot-shard-00-02-bhelp.mongodb.net:'.$this->config[$this->activate]['port'].'/test?ssl=true&replicaSet=Digiebot-shard-0&authSource=admin&retryWrites=true&w=majority';
    ...
    public function where($wheres, $value = null) {
        if (is_array($wheres)) {
            foreach ($wheres as $wh => $val) {
                 if ( ( $wh == '_id' && gettype($val) =='string'  )   || ( $wh == 'buy_order_id' && gettype($val) =='string'  ) || ( $wh == 'order_id' && gettype($val) =='string'  ) ) {
            ...
            if ( ( $value == '_id' && gettype($value) =='string'  )   || ( $value == 'buy_order_id' && gettype($value) =='string'  ) || ( $value == 'order_id' && gettype($value) =='string'  ) ) {
                $this->wheres[$wh] = new MongoDB\BSON\ObjectID($value);
