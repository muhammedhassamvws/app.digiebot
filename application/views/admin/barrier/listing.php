...
?>
<div id="content">
  <h1 class="content-heading bg-white border-bottom">Barrier Listing</h1>
...
?>
    <form class="form-horizontal margin-none" method="post" action="<?php echo SURL ?>admin/reports/barrier-listing" novalidate="novalidate">
	...
	for ($i = 0; $i < count($coins); $i++) {
		...
		?>
          ...
          <div class="col-md-2">
                      <div class="form-group col-md-12">
                        <label class="control-label"></label>
                         <?php $start_date = $session_post_data['start_date'];?>
                     ...
                     <div class="col-md-2">
                      <div class="form-group col-md-12">
                        <label class="control-label"></label>
                        <?php $end_date = $session_post_data['end_date'];?>
...
if (count($barrier_arr) > 0) {
	...
	for ($i = 0; $i < count($barrier_arr); $i++) {
		...
		echo $formated_date_time;?></td>
              ...
              <td class="center"><div class="btn-group btn-group-xs "> <a href="#" data-id="<?php echo $barrier_arr[$i]['_id'] ?>" class="bb btn btn-inverse"><i class="fa fa-pencil"></i></a> <a href="<?php echo SURL . 'admin/reports/delete_barrier/' . $barrier_arr[$i]['_id']; ?>" class="btn btn-danger" onclick="return confirm('Are you sure want to delete?')"><i class="fa fa-times"></i></a>
              <?php if ($barrier_arr[$i]['status'] != 0) {?>
              <a href="<?php echo SURL . 'admin/reports/show_barrier/' . $barrier_arr[$i]['_id']; ?>" data-id="<?php echo $barrier_arr[$i]['_id'] ?>" class="btn btn-success"><i class="fa fa-eye"></i></a>
                              <?php }?>
              </div></td>
            </tr>
            <?php }
...
<div class="modal fade" id="add_form_modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <form class="form-horizontal" id="add_form" method="POST" action="<?=SURL?>admin/reports/add_barrier_action">
...
<script type="text/javascript">
  ...
  $("body").on("click",".bb", function(e){
    ...
    $.ajax({
      ...
      url: "<?php echo SURL ?>admin/reports/edit_barrier/",
