...
<div id="content">
  <h1 class="content-heading bg-white border-bottom">Test Case(TC2 users)</h1>
  <div class="innerAll bg-white border-bottom">
    <ul class="menubar">
      <li class="active" style="background: transparent linear-gradient( 
58deg , #ffffff 0%, #bdbdbd 100%) 0% 0% no-repeat padding-box !important;">Locked users list</li>
    </ul>
  </div>
...
<?php } ?>
<?php $filter_data = $this->session->userdata('filters_users_tc2');
...
<div class="widget widget-inverse">
      <div class="widget-body padding-bottom-none" style="background: #67d2d724 !important;">
          <form method="POST" action="<?php echo SURL; ?>admin/Reports/get_TC2_users" id="users_filter">
          <div class="row">
            ...
            <div class="col-xs-12 col-sm-12 col-md-2 ax_35" style="margin-top: 27px;">
                <div class="Input_text_btn">
                  ...
                  <a href="<?php echo SURL; ?>admin/Reports/resetFilters_tc2"class="btn btn-danger button3">Reset</a>
                  </span>   
                </div>
            </div>
          </div>
 ...
 <!--    <button class="btn nsi_btn button3 pull-right btn-md" type="button" name="csv" id="csv" value="csv" data-toggle="tooltip" title="CSV Report" style="margin-top: 0px;">
    ...
    <div class="widget-body padding-bottom-none">
        ...
        <table class=" table table-bordered table-hover" id="datatable" width="100%"> 
          ...
          <tbody>
<?php
if (count($TC2_users) > 0) {
	for ($i = 0; $i < count($TC2_users); $i++) {
		?>
        <?php if($TC2_users[$i]['tokenStatus'] == 'blacklisted'){ 
         ...
         <tr class="gradeX" <?php echo $danger_class; ?> <?php echo $tooltip; ?> data-id="<?php echo $TC2_users[$i]['_id']; ?>">
              <td><?php echo $i + 1; ?></td>
              <td><?php echo $TC2_users[$i]['user_id']; ?></td>
              <td><?php echo $TC2_users[$i]['first_name']; ?></td>
              <td><?php echo $TC2_users[$i]['last_name']; ?></td>
              <td><?php echo $TC2_users[$i]['username']; ?></td>
              <td><?php echo $TC2_users[$i]['email_address']; ?></td>
              <td><?php echo $TC2_users[$i]['Latest_Locked_date'];?></td>
              <td><?php echo $TC2_users[$i]['exchange']; ?></td>
              <!-- <td><?php //echo //get_price_updated_at((string)$TC2_users[$i]['id']); ?></td> -->
              <td class="">
                <div class="three-dots"></div>
                <div class="btn-group btn-group-md ">
                  <?php $doc_id = $TC2_users[$i]['_id']; if($TC2_users[$i]['is_fixed'] != true){ ?> 
...
$('body').on('click', '#csv', function() {
    $('#tokenfilters').append('<input type="hidden" name="csv" value="csv">');
    $('#tokenfilters').submit();
    $('#tokenfilters').append('<input type="hidden" name="csv" value="">');
});
</script> 
