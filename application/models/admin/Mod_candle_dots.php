<?php
class mod_candle_dots extends CI_Model
{
    ...
    public function get_candelstick_data_from_database($global_symbol, $periods, $from_date_object, $to_date_object, $previous_date, $forward_date)
    {
        ...
        foreach ($responseArr as $val_arr) {
            ...
            $final_arr[] = array(
				...
				'total_volume_percentile' => $val_arr['total_volume_percentile'],
	...
	public function get_candle_report_settings($global_symbol, $start_date_1, $end_date_1){
	     
			$this->mongo_db->limit(1);
			$this->mongo_db->sort(array('openTime_human_readible' => 'ASC'));
			$this->mongo_db->where(array('coin' => $global_symbol,'openTime_human_readible' => $start_date_1));//
			...
			return $candlereport_settings_arr[0];
	}//get_candle_report_settings
	
	public function simulater_report_setting($global_symbol, $start_date_1, $end_date_1,$filter_search_simulater_sett){
		    
			$this->mongo_db->limit(10);
			//$this->mongo_db->sort(array('created_date' => 1));
			$this->mongo_db->where(array('settings.title_to_filter' => $filter_search_simulater_sett));//
			$get_simulater_report_setting = $this->mongo_db->get('meta_coin_report_results');//meta_coin_report_results
			$simulater_report_setting_arr = iterator_to_array($get_simulater_report_setting);	
			//echo "<pre>";  print_r($simulater_report_setting_arr); 	
			return $simulater_report_setting_arr[0]->result;
	}//simulater_report_setting
	
	
	public function report_setting_simulater($global_symbol){
		
		$this->mongo_db->sort(array('created_date' => -1));
	    $this->mongo_db->where(array('trigger' => 'percentile','filter_by_coin' =>$global_symbol));
        $sett = $this->mongo_db->get("report_setting_collection");
        $data = iterator_to_array($sett);
		
		return $data;
	}//report_setting_simulater
