<?php
/** Mod_candle_chart **/
class mod_candle_chart extends CI_Model {
    public function get_candelstick_data_from_database($global_symbol, $periods, $from_date_object, $to_date_object, $previous_date, $forward_date) {
        ...
        foreach ($responseArr as $val_arr) {
            $final_arr[] = array(
                ...
                'black_wall_percentile' => $val_arr['blackwall_percentile'],
                'seven_level_percentile' => $val_arr['sevenlevel_percentile'],
                'ask_val_percentile' => $val_arr['ask_percentile'],
                'bid_val_percentile' => $val_arr['bid_percentile'],
                'fivemin_percentile' => $val_arr['fivemin_percentile'],
                'last_qty_buy_vs_sell_val_percentile' => $val_arr['last_qty_buy_vs_sell_percentile'],
                'last_qty_time_ago_val_percentile' => $val_arr['last_qty_time_ago_percentile'],
                'fivemin_b_percentile' => $val_arr['fivemin_b_percentile'],
                'last_qty_sell_vs_buy_val_percentile' => $val_arr['last_qty_sell_vs_buy_percentile'],
            );
        } //$responseArr as $val_arr
        if ($forward_date == '') {
    ...
    public function get_market_history_for_candel($symbol) {
        ...
        if (count($market_history_Arr_ask) > 0) {
            ...
            } //$market_history_Arr_ask as $key => $value
        } //count($market_history_Arr_ask) > 0
        $match = array(
            '$match' => array(
                'type' => 'bid',
                'coin' => $symbol,
        ...
        if (count($market_history_Arr_bit) > 0) {
            ...
            } //$market_history_Arr_bit as $key => $value
        } //count($market_history_Arr_bit) > 0
    ...
    public function get_candle_price_volume_detail($symbol, $start_date, $end_date, $unit_value) {
        ...
        if (count($ask_arr_volume) > 0) {
            ...
            } //$ask_arr_volume as $price => $valume
        } //count($ask_arr_volume) > 0
        foreach ($bid_arr_volume as $bid_price => $bid_volume) {
            ...
            } //!array_key_exists($bid_price, $total_volume_arr)
        } //$bid_arr_volume as $bid_price => $bid_volume
    ...
    public function get_hour_volume_array_detail($symbol, $start_date, $end_date) {
        ...
        if (count($ask_arr_volume_hourly) > 0) {
            ...
            } //$ask_arr_volume_hourly as $date => $valume
        } //count($ask_arr_volume_hourly) > 0
        foreach ($bid_arr_volume_hourly as $bid_date => $bid_volume) {
            ...
            } //!array_key_exists($bid_date, $total_volume_arr)
        } //$bid_arr_volume_hourly as $bid_date => $bid_volume
        $max_volume = max($total_volume_arr);
    ...
    public function get_ask_price_volume_for_hour($symbol, $start_date, $end_date, $type) {
        ...
        $connect = $this->mongo_db->customQuery();
        $market_history_Arr_ask = $connect->fifteen_min_market_history->aggregate(array(
            $project,
            $match,
            $pipeline,
            $sort,
    ...
    public function get_order_array($symbol, $admin_id, $global_mode) {
        ...
        foreach ($buy_orders_arr as $key => $arr) {
            ...
            } //$sold_order_arr as $key => $value
        } //$buy_orders_arr as $key => $arr
        return $final_arr;
    } //end get_order_array
    ...
    public function calculate_up_down_wall($coin_symbol, $datetime) {
        ...
        for ($i = 0; $i < count($array); $i++) {
            if (!empty($array[$i]['current_market_value'])) {
                ...
                $x++;
            } //!empty($array[$i]['current_market_value'])
        } //$i = 0; $i < count($array); $i++
    ...
    public function get_all_indicators_from_last_week($coin_symbol) {
        $start_date = date("Y-m-d H:00:00", strtotime($date));
        $search_array = array(
            'coin' => $coin_symbol);
        $this->mongo_db->where($search_array);
    ...
    public function get_all_indicators($coin_symbol, $start_date, $end_date) {
        ...
        rsort($volume_Arr);
        $ret_arr['blackwall'] = $volume_Arr; //array_sum($volume_Arr)/count($volume_Arr);

        $volume_Arr2 = array_column($ask_volume_arr, 'sellers_buyers_per');
        rsort($volume_Arr2);
        $ret_arr['sellers_buyers_per'] = $volume_Arr2; //array_sum($volume_Arr)/count($volume_Arr);

        $seven_level_depth = array_column($ask_volume_arr, 'seven_level_depth');
        rsort($seven_level_depth);
        $ret_arr['sevenlevel'] = $seven_level_depth; //array_sum($seven_level_depth)/count($seven_level_depth);

        $market_depth_quantity = array_column($ask_volume_arr, 'market_depth_quantity');
        rsort($market_depth_quantity);
        $ret_arr['bid_quantity'] = $market_depth_quantity; //array_sum($market_depth_quantity)/count($market_depth_quantity);

        $last_qty_buy_vs_sell = array_column($ask_volume_arr, 'last_qty_buy_vs_sell');
        rsort($last_qty_buy_vs_sell);
        $ret_arr['last_qty_buy_vs_sell'] = $last_qty_buy_vs_sell; //array_sum($market_depth_ask)/count($market_depth_ask);

        $last_200_buy_vs_sell = array_column($ask_volume_arr, 'last_200_buy_vs_sell');
        rsort($last_200_buy_vs_sell);
        $ret_arr['last_200_buy_vs_sell'] = $last_200_buy_vs_sell; //array_sum($market_depth_ask)/count($market_depth_ask);

        $last_qty_time_ago = array_column($ask_volume_arr, 'last_qty_time_ago');
        rsort($last_qty_time_ago);
        $ret_arr['last_qty_time_ago'] = $last_qty_time_ago; //array_sum($market_depth_ask)/count($market_depth_ask);

        $market_depth_ask = array_column($ask_volume_arr, 'market_depth_ask');
        rsort($market_depth_ask);
        $ret_arr['ask_quantity'] = $market_depth_ask; //array_sum($market_depth_ask)/count($market_depth_ask);
        // echo "<pre>";
        // print_r($ret_arr);
        // exit;
        return $ret_arr;
