<?php
class mod_test_trigger extends CI_Model {
	...
	public function get_parent_orders($coin_symbol) {
		...
		$this->mongo_db->where(array('status' => 'new', 'trigger_type' => 'trigger_test', 'symbol' => $coin_symbol, 'parent_status' => 'parent'));
	...
	public function get_profit_sell_orders($coin_symbol) {
		$this->mongo_db->where_ne('is_sell_order', 'sold');
		$this->mongo_db->where_ne('parent_status', 'parent');
		$this->mongo_db->where_in('order_mode', array('test_live', 'live'));
		$this->mongo_db->where(array('status' => 'FILLED', 'trigger_type' => 'trigger_test', 'symbol' => $coin_symbol));
	...
	public function get_stop_loss_updating_orders($trigger_type, $target_sell_price, $coin_symbol) {
		$this->mongo_db->where_ne('is_sell_order', 'sold');
		$this->mongo_db->where_in('order_mode', array('test_live', 'live'));
		$this->mongo_db->where(array('status' => 'FILLED', 'trigger_type' => $trigger_type, 'symbol' => $coin_symbol,'is_market_deep_range'=>'yes'));
		$buy_orders_result = $this->mongo_db->get('buy_orders');
		return iterator_to_array($buy_orders_result);
	...
	public function is_market_deep_ready_stop_loss_update($coin_symbol, $sell_profit_percet, $current_market_price)
	{
		$diff_price = $current_market_price - ($current_market_price / 100) * $sell_profit_percet;
		$trigger_type = 'barrier_trigger';

		$orders_arr = $this->make_stop_loss_ready_to_update($trigger_type, $diff_price, $coin_symbol);
	...
	public function update_stop_loss_close_to_deep_price($market_deep_price,$iniatial_trail_stop,$id,$admin_id){
			$created_date = date('Y-m-d G:i:s');
			$deep_difference_price  = (float) $market_deep_price - ($market_deep_price / 100) *.1;
			if($iniatial_trail_stop < $deep_difference_price){
				...
				$log_msg = "Stop Loss Update From <b>".num($iniatial_trail_stop)."</b>   To ".num($deep_difference_price)." By Rule <span style='color:green'> <b>Colse 0.1 % </b></span>To deep Price";
				$this->insert_order_history_log($id, $log_msg, 'buy_created', $admin_id, $created_date);
				/*%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%*/
			}
			return true;
	...
	public function make_stop_loss_ready_to_update($trigger_type, $diff_price, $coin_symbol) {
		$this->mongo_db->where_ne('is_sell_order', 'sold');
		...
		$this->mongo_db->where_lte('market_deep_price', $diff_price);
		$buy_orders_result = $this->mongo_db->get('buy_orders');
		return iterator_to_array($buy_orders_result);
	} //End of make_stop_loss_ready_to_update

	public function stop_loss_big_wall_barrier_trigger($coin_symbol, $sell_profit_percet, $current_market_price) {
		...
		$orders_arr = $this->get_stop_loss_updating_orders($trigger_type, $target_sell_price, $coin_symbol);
		...
		if (count($orders_arr) > 0) {
			foreach ($orders_arr as $data) {
				...
				if($market_deep_true){
					if($level_five_big_wall_price > $two_percent_up_deep_range){
					...
					}else if($current_market_price >= $two_percent_up_deep_range){
						...
						//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
					}else if($level_five_big_wall_price < $one_percent_up_deep_range){
						//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
						if (($level_five_big_wall_price > $iniatial_trail_stop)) {
	...
	{	
		...
		$insert_arr = array('coin_symbol'=>$coin_symbol,'rule_number'=>$rule_no,'type'=>$type,'market_price'=>$market_price,'log'=>$new_message,'created_date'=>$created_date);
