...
class mod_api_calls extends CI_Model {
	...
	public function get_coin_balance($user_id, $coin_symbol, $exchange){
		...
		if(empty($collection)){
			...
			if($exchange == 'kraken' || $exchange == 'dg'){
	...
	public function get_all_user_coins($user_id, $exchange='') {
		...
		// ];
		if ($user_id == '63246dedaaca50474c28c9d2') {
			$where_arr = [
		...
		// echo '<pre>where array :: '; print_r($where_arr); exit;
	...
	//get_all_user_coins //Hassam
	public function get_all_user_coins_btc_only($user_id, $exchange='') {

		$where_arr = [
			'user_id' => 'global',
			'symbol' => [
				'$nin' => ['', null, 'BTC', 'NCASHBTC', 'BTCUSDT'],
				'$regex' => 'BTC',
				'$options' => 'i'
			]
		];
	...
	public function get_open_trades($user_id, $symbol, $exchange='', $application_mode='live', $trigger_type=NULL) {
		...
		$this->mongo_db->where($where)->select($projection);
		$responseArr = $this->mongo_db->get($collection);
	...
	public function get_sold_trades($user_id, $symbol, $exchange='', $application_mode='live', $trigger_type=NULL) {
	...
	public function get_costAvg_child_trades($user_id, $symbol, $exchange='', $application_mode='live', $trigger_type=NULL) {
		//CostAvg Filter
		$where = [
			'admin_id' => $user_id,
			'application_mode' => $application_mode,
			'symbol' => $symbol,
			'status' => ['$ne'=>'canceled'],
			'cost_avg'=> ['$in'=> ['taking_child', 'yes','CA_TAKING_CHILD','ca_taking_child']],
	...
	public function get_24_hour_price_change($symbol, $exchange='') {
		$coin_nature = substr($symbol,-3); // returns the last three characters of string.
		if($coin_nature == 'BTC'){
		 	$symbol_new = str_replace("BTC","",$symbol); // will replace BTC with space if the symbol is containing word BTC.
		}else{
			$symbol_new =  str_replace("USDT","",$symbol); // will replace BTC with space if the symbol is containing word BTC.
		}
		$this->mongo_db->where(array('symbol' => $symbol_new));
        $res = $this->mongo_db->get('coin_price_change_new');
        $result_arr = iterator_to_array($res);

        return array(
			'change' => 0,
			'percentage' => number_format($result_arr[0]['changePercent24Hr'], 2)
		);
