...
class mod_report extends CI_Model {
    ...
    public function get_admin_ids_excluding($user_names_array) { // function by sheraz to exclude users from order report

        $this->mongo_db->where(array('username' => ['$in'=>$user_names_array]));
        $get_users = $this->mongo_db->get('users');
        //echo $this->db->last_query();
        $user_arr = iterator_to_array($get_users);
        return $user_arr;

    } //end get_users_to_exclude_from report
    public function get_admin_ids_excluding_investment_blocked($exchange){
        if($exchange == 'binance'){
            $collection_name ='user_investment_binance';
        }else{
        ...
        $get_users = $db->$collection_name->aggregate($piepline);
        $user_arr = iterator_to_array($get_users);
        return $user_arr;
    } 
    ...
    public function get_all_testcase_users_for_csv($data_filters){
        $custom = $this->mongo_db->customQuery();
        $get_tokens = $custom->test_cases_users->find($data_filters);
        $data = iterator_to_array($get_tokens);
        $final_arr = array();
        for ($i=0; $i < count($data) ; $i++) // for loop for sending array to csv helper function
            { 
                $finalArr[$i]['sr.'] = $i+1;    
                $finalArr[$i]['User id'] = (string)$data[$i]['user_id'];    
                $finalArr[$i]['First name'] = (string)$data[$i]['first_name'];    
                $finalArr[$i]['Last name'] = (string)$data[$i]['last_name'];    
                $finalArr[$i]['Username'] = (string)$data[$i]['username'];    
                $finalArr[$i]['Email Address'] = (string)$data[$i]['email_address'];    
                $finalArr[$i]['Symbol'] = (string)$data[$i]['symbol'];   
                $finalArr[$i]['Orders Found'] = (string)$data[$i]['orders_count'];       
                $finalArr[$i]['Exchange'] = (string)$data[$i]['exchange'];   
            } // end for loop.
        return $finalArr;
    }    
